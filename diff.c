...
int git_diff_ui_config(const struct config_context *ctx, void *cb)
{
	...
	return git_diff_basic_config(ctx, cb);
}

int git_diff_basic_config(const struct config_context *ctx, void *cb)
{
	const char *var = ctx->key;
	const char *value = ctx->value;
	const char *name;
